...
# trip minutes by membership + weekday
df %>% 
  group_by(member_casual, weekday) %>% 
  summarise(across(trip_minutes, c(min=min, max=max, mean=mean, median=median, sd=sd))) %>% 
  ggplot(aes(weekday, trip_minutes_mean, fill = member_casual)) +
  geom_col(position = "dodge") +
  labs(title = "Average Length of Trip over the week",
       x = NULL, y = "Minutes",
...
df %>% 
  ...
  geom_point(aes(color = factor(mostly_casual, 
                                levels = c(TRUE, FALSE),
  ...
  labs(x = 'Popularity of station', y = '% who are casuals',
       ...
       caption = "Source: Divvy public data") +
  annotate("text", x = -660, y = 87, label = "The busiest stations are\n mostly used casually") +
  annotate("curve", x = -550, y = 90, xend = -100, yend = 95, curvature = 0.1,
           arrow = arrow(length = unit(0.04, "npc"))) +
  annotate("curve", x = -550, y = 90, xend = -70, yend = 28, curvature = -0.1,
           arrow = arrow(length = unit(0.04, "npc"))) +
  coord_flip() +
  theme(legend.title = element_blank())
```
