...
# stations binned into sectors: points don't overlap now
ggplot(boundaries_chi, aes()) +
  geom_sf() +
...
Let's examine the difference in overall traffic flow over each hour in a week,
and compare members vs casual users.

```{r create motion vectors day-of-week-hour}
...
We see the members commuting signal very strongly here.  Casual users also
have patterns visible:

- On the weekends there seems to be traffic flowing northeast and west at
different times of the day.
- During the week there is a weak signal that corresponds with the typical
commuting directions.

Let's break this down further and split the trips into weekends vs weekdays.

```{r create motion vectors weekday-hour}
...
We see the commuting pattern during the week, as expected. 
On the weekend however, many casual trips are taken in the morning to
early afternoon, traveling
northeast.  In the afternoon the overall casual traffic flow swings to
