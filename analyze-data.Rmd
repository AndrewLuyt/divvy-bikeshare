...
```
```{r mapping the dataset 8, echo=FALSE, message=FALSE}
...
ggplot(boundaries_chi, aes()) +
  ...
  labs(title = 'Mean duration of trips started here')
```
```{r mapping the dataset 11, echo=FALSE, message=FALSE}
...
boundaries_chi %>% ggplot() +
  ...
  guides(color = guide_legend(reverse = TRUE)) +  # reverse legend order, TRUE first
  labs(title = "All stations over 70% casual use",
...
boundaries_chi %>% ggplot() +
  geom_sf() +
  geom_point(data = stations_users, 
             mapping = aes(x = station_lng, y = station_lat, size = n_trips_station),
             color = "red", alpha = 0.8) +
  guides(color = guide_legend(reverse = TRUE)) +  # reverse legend order, TRUE first
