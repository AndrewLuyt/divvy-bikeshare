...
x <- df %>% 
  select(start_lng, start_lat, start_lng_sector, start_lat_sector, 
         end_lng, end_lat, end_lng_sector, end_lat_sector, member_casual, 
         trip_minutes, is_round_trip) #%>% 
...
boundaries_chi %>% ggplot(aes()) + 
  geom_sf() +
  geom_bin2d(data = x, mapping = aes(start_lng, start_lat)) +
  scale_fill_viridis(option = 'plasma') +
  facet_wrap(~member_casual) +
...
# 42 & 25 are the # of rectangular sectors
ggplot(boundaries_chi, aes()) + 
    # geom_sf() +
    geom_bin2d(data = x,
            mapping = aes(start_lng, start_lat)) +
...
- Calculate mean straight-line **distance** of trips started and ended in each
sector.
