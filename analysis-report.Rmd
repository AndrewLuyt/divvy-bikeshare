...
stations_users %>% 
  ...
  ggplot() +
  geom_sf(data = boundaries_chi, inherit.aes = FALSE) +
  geom_point(mapping = aes(x = station_lng, y = station_lat, size = n_trips_station),
             color="black", fill = "red", pch=21) +
  scale_size_continuous(range = c(2,9)) +
  labs(title = "Stations with over 70% casual use are highlighted",
...
p <- 
  ggplot(volume, 
         aes(x = start_lng,
             ...
             fill = p_casual,
             color = p_casual)) +
  geom_sf(data = boundaries_chi, mapping = aes(), inherit.aes = FALSE, fill="grey75") +
  geom_point(pch = 21) + #, color='grey70') +
  scale_fill_distiller(palette = "RdYlBu", name = "Casual users", labels = scales::percent) +
  scale_color_distiller(palette = "RdYlBu", guide = "none") +  # turn off a legend with guide=FALSE
  scale_size_continuous(range = c(1, 9), guide = "none") +
  annotate("text", x = -87.865, y = 42.06, label = "CHICAGO", color = "grey70", cex = 10) +
  labs(subtitle = "Red means a high percentage of casual users leaving a station, blue means users\nwith Divvy memberships.   Larger circles mean more trips starting there.",
       caption = "Andrew Luyt, 2021  |  Source: Divvy public data") +
  ggtitle("Divvy bike share: traffic volume & user type     {next_state}") +
  theme(axis.title = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank(),
...
FPS = 24; W = 600; H = 660; DTL = 2; NFRAMES = 6*24*3; S = paste0(W,"x",H)
...
anim_save("img/traffic-volume-map.gif", p, fps = FPS, nframes = NFRAMES, width=W, height=H)
```
...
FPS = 24; W = 650; H = 680; NFRAMES = 6*24*3 # bins per hour * 24 hours * (transition + state)
animate(plot = p, fps = FPS, nframes = NFRAMES, width=W, height=H)
