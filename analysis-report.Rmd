...
animate(plot = p, fps = 24, nframes = 400, detail=2)
# anim_save("img/trafficflow-map.gif", p)
```


# TODO: group by member-casual and colour each?  Or colour all by proportion
of casuals.
...
volume <- df %>% 
  ...
  summarise(n = n()) %>% 
  mutate(total_trips = sum(n),
         p_casual = n / total_trips * 100) %>% 
  filter(member_casual == "casual") %>% 
  mutate(mostly_casual = factor(p_casual > .5,
                                levels = c(TRUE, FALSE),
                                labels = c("Mostly casual", "Mostly members"))) %>% 
  arrange(timecode, total_trips)  # for overplotting: last to be plotted are the largest points
  # arrange(timecode, p_casual)  # for overplotting: last to be plotted are the highest casual %
  # arrange(timecode, p_casual * total_trips)  
...
p <- 
  ...
  labs(subtitle = "Trips starting at each station") +
  # ylim(c(41.77, 42.01)) +
  # xlim(c(-87.8, -87.57)) +
  transition_states(states = timecode, transition_length = 0, state_length = 1, wrap = TRUE) +
  ggtitle("Typical Divvy Traffic Volume: {next_state}") +
  # exit_fade(alpha = .5)
  shadow_wake(wake_length = 0.05, wrap = TRUE, alpha = 0.1, size=1)

animate(plot = p, fps = 15, nframes = 288)
...
alltraffic_fine <- df %>% 
  ...
  mutate(angle = cut_interval(angle, n = 4)) %>%  
...
p <- 
  ggplot(alltraffic_fine, 
         aes(x = start_lng,
             y = start_lat, 
             xend = start_lng + sign(x) * log2(abs(x))/800,
             yend = start_lat + sign(y) * log2(abs(y))/800,
             color = angle)) +
  geom_sf(data = boundaries_chi, mapping = aes(), inherit.aes = FALSE) +
  geom_segment(size=0.6, arrow = arrow(length = unit(0.004, "npc"))) +
  theme(legend.position = "none",
        axis.title = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank()) +
  ...
  # enter_grow(size = .2) 
  shadow_wake(wake_length = 0.07,  wrap = TRUE)

FPS = 20; W = 600; H = 660; NFRAMES = 6*24*2 # bins per hour * 24 hours * extra frames
...
```{r}
anim_save("img/trafficflow-map-detailed.gif", p, fps = FPS, nframes = 6*24*2, width=W, height=H)
