...
We can clearly see a division between casual use and member use. Next let's 
highlight hot spots.  We'll plot only stations with over 70% casual 
...
Finally let's zoom in on the notable hot spot near Navy Pier.  Of all stations 
with over 70% casual use, these are the busiest.
...
patterns.  First, we will animate the traffic volume at all stations
...
Next we'll examine the average traffic patterns in June and compare weekdays
 & weekends.
...
FPS = 30;W = 800;H = 600;DTL = 3;S = paste0(W,"x",H);NFRAMES = 3*24*1*5 # minutebins*hours*days*framesperbin
animate(plot = p, fps = FPS, nframes = NFRAMES, width = W, height = H, detail = DTL)
# anim_save("img/traffic-volume-map.mp4", p, fps = FPS, nframes = NFRAMES,
#           width=W, height=H, detail=DTL,
#           renderer = ffmpeg_renderer(options = list(s = S, b = "1200k")))
...
- On weekdays traffic starts earlier and that early morning traffic is primarily 
...
Let's remind ourselves of the overall traffic volume patterns over a year.

```{r traffic volume linegraph}
volume <- df %>% 
  select(started_at, member_casual, weekend_weekday) %>% 
  mutate(minute = minute(started_at),     # round time into bins
         minute = cut_interval(minute, n = 3, 
                               labels = c("00","20","40")),
         started_at =
  ...
  ungroup() %>%   
  # Turn totals into average figures.  There are 22 weekdays and 8 weekends in June 2021
  arrange(started_at, total_trips)

volume %>% ggplot(aes(started_at, n, color = member_casual)) +
  geom_line()
```

Note how low the traffic is in winter, particularly for casual users. Next
we will map the average winter traffic volume, to compare with June.
...
Each arrow represents the averaged
motion of all the bikes during a 10-minute period.  An arrow pointing
northwest means that during that time the average motion of bikes leaving
...
We'll also show data at all the individual bike stations, rather than a
...
p <- 
  ...
  ggtitle("Bicycle traffic flow    {next_state}") +
  shadow_wake(wake_length = 0.02,  wrap = TRUE)
