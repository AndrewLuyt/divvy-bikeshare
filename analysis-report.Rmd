...
one is on.
```{r all traffic flow mapped}
alltraffic <- df %>% 
  group_by(timecode = 
             paste0(sprintf("%02d", lubridate::hour(started_at)), ":",
                    if_else(lubridate::minute(started_at) <= 30, "00", "30")),
  ...
  mutate(angle = angle_from_x_axis(y,x)) %>% 
  ungroup() %>% 
  mutate(angle = cut_interval(angle, n = 4)) %>%  # bin angles into 4 quadrants
  filter(magnitude >= 50)  # higher numbers make the map less cluttered
# alltraffic$angle = cut(alltraffic$angle - 30, breaks = 4)
...
p <- 
  ggplot(alltraffic, 
         aes(x = start_lng_sector,
             y = start_lat_sector, 
             xend = start_lng_sector + sign(x)*(log10(abs(x)))**1.1/250,
             yend = start_lat_sector + sign(y)*(log10(abs(y)))**1.1/250,
             color = angle)) +
  geom_sf(data = boundaries_chi, mapping = aes(), inherit.aes = FALSE) +
  geom_segment(arrow = arrow(length = unit(0.03, "npc")), size=1.6) +
  theme(legend.position = "none",
        axis.title = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank()) +
  # scale_color_viridis(option = "turbo", discrete = TRUE, direction = -1) +
  # scale_color_fermenter(palette = "Dark2", direction = -1, ) +
  scale_color_brewer(palette = "Set1") +
  labs(subtitle = "Colours show direction of traffic flow") +
  ylim(c(41.77, 42.01)) +
  xlim(c(-87.8, -87.57)) +
  transition_states(states = timecode, transition_length = 0, state_length = 2) +
  ggtitle("Typical Divvy Traffic Flow: {next_state}")

animate(plot = p, fps = 3, nframes = 48)
# anim_save("img/trafficflow-map.gif", p)
```

```{r sectored weekend vectors}
...
weekend <- df %>% 
  ...
  group_by(timecode = paste0(sprintf("%02d", lubridate::hour(started_at)), ":",
                             if_else(lubridate::minute(started_at) <= 30, "00", "30")),
           start_lng_sector = Sector(start_lng_sector), 
           start_lat_sector = Sector(start_lat_sector)) %>% 
  summarize(x = sum(trip_delta_x), 
  ...
  rowwise() %>% 
  mutate(angle = angle_from_x_axis(y,x)) %>% 
  filter(magnitude >= 15)  # higher numbers make the map less cluttered
weekend$angle = cut(weekend$angle - 30, breaks = 4)
...
p <- 
  ggplot(weekend, 
         aes(x = start_lng_sector,
             y = start_lat_sector, 
             xend = start_lng_sector + sign(x)*log10(abs(x))/200,
             yend = start_lat_sector + sign(y)*log10(abs(y))/200,
             color = angle)) +
  geom_sf(data = boundaries_chi, mapping = aes(), inherit.aes = FALSE) +
  geom_segment(arrow = arrow(length = unit(0.03, "npc")), size=1.6) +
  theme(legend.position = "none",
  ...
  xlim(c(-87.8, -87.57)) +
  transition_states(states = timecode, transition_length = 0, state_length = 2) +
  ggtitle("Typical Weekend Traffic Flow: {next_state}")

animate(plot = p, fps = 3, nframes = 48)
# anim_save("img/trafficflow-map.gif", p)
```
```{r sectored weekday vectors}
weekday <- df %>% 
  ...
  rowwise() %>% 
  mutate(angle = angle_from_x_axis(y,x)) %>% 
  filter(magnitude >= 20)  # higher numbers make the map less cluttered
weekday$angle = cut(weekday$angle - 30, breaks = 4)
...
p <- 
  ...
  xlim(c(-87.8, -87.57)) +
  transition_states(states = timecode, transition_length = 0, state_length = 2) +
  ggtitle("Typical Weekday Traffic Flow: {next_state}")
