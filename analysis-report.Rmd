...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, fig.align = "left", fig.width=5, fig.height=4)
```
...
![Animated map of averaged bicycle traffic volume in Chicago in June showing regular patterns of activity](analysis-report_files/figure-gfm/all traffic volume mapped june weekday-1.gif)
...
- There are four distinct patterns of [regional traffic flow](#average-traffic-flow) 
that divide the city into quarters.
- There is a very large [seasonal variation](#seasonal-variation) in ridership.
...
- 10% of all rides are probably for [enjoyment and exercise](#a-special-case-pleasure-cruises)
  ...
  - More than 1 in 7 casual passes are probably bought for this purpose
...
```{r all traffic volume mapped, fig.alt = "Animated map of averaged bicycle traffic volume in Chicago on June 19 showing complex patterns of activity"}
volume <- df %>% 
...
# anim_save("export/traffic-volume-map.mp4", p, fps = FPS, nframes = NFRAMES,
#           width=W, height=H, detail=DTL,
#           renderer = ffmpeg_renderer(options = list(s = S, b = "1200k")))
# anim_save("export/traffic-volume-map.gif", p, fps = FPS, detail = DTL, nframes = NFRAMES, width=W, height=H)
```
...
```{r casual usage map, fig.alt = "Map of user types at all bike stations in Chicago 19 showing distinct geographical differences", fig.align='left', fig.width=4.5}
...
chicago_map %>% ggplot() +
  ...
  geom_point(data = stations_users, 
             ...
             size = 2.2) +
  scale_fill_viridis(option = "viridis", labels = scales::percent) +
  labs(title = "Average user type at each station",
       fill = "Casual use\n") +
  theme_map
```
...
```{r member vs casual usage map over 70, fig.alt = "Map of bike stations with high proportions of member or casual user showing distinct geographical differences"}
...
```{r member vs casual usage map over 70 zoom, fig.width=9.5, fig.height = 6, fig.alt = "Map of high volume bike stations clustered on Chicago's waterfront"}
...
A clear division between stations popular with members and those popular with
casual users is evident.
...
we see that members take the majority of rides, about 30% more. 

```{r who rides most often, fig.width=5, fig.height=3, fig.alt="Bar graph of total rides by user type showing members ride 30% more often"}
df %>% 
...
```{r who rides most often by hour, fig.width=7, fig.height=4, fig.alt="Bar graph of total rides by user type and hour showing volume spikes from commuting"}
df %>% 
...
commutes. Between 6 and 8 in the morning members make up at least 70% of the
ridership.

#### Rides by weekday

```{r who rides most often by weekday, fig.width=7, fig.height=3, fig.alt="Bar graph of total rides by user type and weekday showing a weekend doubling in casual users"}
df %>% 
...
Members ride about the same amount through the week while
casual use doubles on the weekend. 

### Weekdays vs Weekends
Here we explore the increase in weekend casual use by comparing weekdays and 
weekends in June. Colours will display the average type of rider.

```{r all traffic volume mapped june weekday, fig.alt="Animated map of averaged bicycle traffic volume in Chicago in June showing complex patterns of activity"}
...
animate(plot = p, fps = FPS, nframes = NFRAMES, width = W, height = H, detail = DTL)
# anim_save("export/traffic-volume-map-weekday-weekend.mp4", p, fps = FPS, nframes = NFRAMES,
#           width=W, height=H, detail=DTL,
#           renderer = ffmpeg_renderer(options = list(s = S, b = "1200k")))
# anim_save("export/traffic-volume-map-weekday-weekend.gif", p, fps = FPS, detail = DTL, nframes = NFRAMES, width=W, height=H)
```
...
members, possibly on the morning commute. Member usage stays proportionally higher all 
...
```{r who rides most often 3, fig.width=7, fig.height=3, fig.alt="Bar graph of total rides by user type and month showing large seasonal variation"}
df %>% 
...
```{r all traffic volume mapped winter, fig.alt="Animated map of averaged bicycle traffic volume in Chicago in winter showing lower volume and lower casual use"}
volume <- df %>% 
...
animate(plot = p, fps = FPS, nframes = NFRAMES, width = W, height = H, detail = DTL)
# anim_save("export/traffic-volume-map-winter.mp4", p, fps = FPS, nframes = NFRAMES,
#           width=W, height=H, detail=DTL,
#           renderer = ffmpeg_renderer(options = list(s = S, b = "1200k")))
# anim_save("export/traffic-volume-map-winter.gif", p, fps = FPS, detail = DTL, nframes = NFRAMES, width=W, height=H)
```
...
bikes in a region during a 10-minute period. The stronger the trend, the longer
the arrow.

*Technical note: to calculate this metric, each trip is treated as a vector
from start station to end station.
All vectors in a particular region in a particular period of time are added 
end-to-end and the final vector is treated as the overall traffic flow.*

```{r all traffic flow mapped sectored, fig.alt="Animated map of averaged bicycle traffic directions in Chicago showing a clear and simple pattern of activity"}
alltraffic <- df %>% 
  filter(is_round_trip == FALSE) %>%
...
FPS = 30; W = 560; H = 660; DTL = 2; NFRAMES = 6*24*4 # bins per hour * 24 hours * ()
animate(plot = p, fps = FPS, nframes = NFRAMES, detail=DTL, width=W, height=H)
# anim_save("export/traffic-flow-sectored.mp4", p, fps = FPS, nframes = NFRAMES,
#           detail = DTL, width=W, height=H,
#           renderer = ffmpeg_renderer(options = list(b = "1200k")))
# anim_save("export/traffic-flow-sectored.gif", p, fps = FPS, nframes = NFRAMES, 
#           detail = DTL, width=W, height=H)
```
...
either to the northwest or southwest, depending how far north the riders are.
...
```{r all traffic flow mapped fine detail zoomed, fig.alt="Animated map of detailed bicycle traffic directions in Chicago showing a clear pattern of activity"}
...
FPS = 30; W = 680; H = 680; NFRAMES = 6*24*4 # bins per hour * 24 hours * (transition + state)
animate(plot = p, fps = FPS, nframes = NFRAMES, width=W, height=H)
# anim_save("export/traffic-flow-zoom.mp4", p, fps = FPS, nframes = NFRAMES,
#           width=W, height=H,
#           renderer = ffmpeg_renderer(options = list(b = "1200k")))
# anim_save("export/traffic-flow-zoom.gif", p, fps = FPS, nframes = NFRAMES, width=W, height=H)
```
...
For the traffic flow analysis we removed trips that start and end 
...
```{r pleasure cruises seasonal, fig.width = 7, fig.height=3.5, fig.alt="Bar graph of pleasure cruises by user type and month showing casual users greatly outnumbering members"}
df %>% 
...
```{r pleasure cruises seasonal duration, fig.width = 7, fig.height=3, fig.alt="Bar graph of pleasure cruises by user type and month showing casual users riding much longer"}
df %>% 
  group_by(month, member_casual, is_round_trip) %>% 
...
```{r pleasure cruises mapped, fig.alt="Map of location and duration of pleasure cruises showing large casual use along waterfront", fig.width=7, fig.height=5}
pc <- df %>% 
  filter(is_round_trip == TRUE) %>% 
...
inland from the waterfront. Finally we'll animate the average duration and locations
of these cruises.

```{r pleasure cruises volume animated, fig.alt="Animated map of bicycle pleasure cruises showing most activity along Chicago's waterfront and south of the river"}
volume <- pc %>% 
  # busiest day of the year is day 170 (June 19)
  # filter(lubridate::year(started_at) == 2021, lubridate::yday(started_at) == 170) %>%
  # filter(lubridate::hour(started_at) > 11, lubridate::hour(started_at) < 15) %>%
...
though perhaps more concentrated on the waterfront.
...
```{r avg trip distance, fig.width=5, fig.height=3, fig.alt="Bar graph showing members and casuals both averaging about 2.5 kilometers per ride"}
df %>% 
  filter(is_round_trip == FALSE) %>% 
  group_by(member_casual) %>% 
...
```{r mean ride duration, fig.width=5, fig.height=3, fig.alt="Bar graph showing casuals riding twice as long on average than members"}
df %>% 
  group_by(member_casual) %>% 
...
```{r mean ride duration distribution, fig.width=5, fig.height=3, fig.alt="Distribution plot showing almost all rides longer than 45 minutes are by casual users"}
df %>% 
  ...
  annotate("text", x = 65, y = 0.015, label = "45 minutes", color='grey40') +
...
```{r mean ride duration seasonal, fig.width=6, fig.height=3, fig.alt="Bar graph of ride duration by month showing large variation in casual ride time peaking in summer"}
df %>% 
  group_by(member_casual, month) %>% 
  summarise(mean_ride_length = mean(trip_minutes)) %>% 
...
```{r mean trip speed, fig.width=5, fig.height=3, fig.alt="Bar graph of ride speed showing members ride 30% faster on average"}
df %>% 
  filter(is_round_trip == FALSE) %>%  # round trips have a distance of 0
...
```{r saddle time, fig.width=5, fig.height=3, fig.alt="Bar graph showing casuals have spent 76% more time riding in total than members"}
