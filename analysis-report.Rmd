...
```{r bin2d traffic volume, echo=FALSE, message=FALSE}

```


```{r bin2d traffic membercasual, echo=FALSE, message=FALSE}
volume <- df %>% 
  # busiest day of the year is day 170 (June 19)
  filter(lubridate::year(started_at) == 2021, lubridate::yday(started_at) == 170) %>%
  # filter(lubridate::hour(started_at) > 10, lubridate::hour(started_at) < 12) %>%
  mutate(minute = minute(started_at),    # round time into bins
         minute = cut_interval(minute, n = 3,
                               labels = c("00","20","40")),
         started_at = 
           parse_date_time(paste0(lubridate::date(started_at), " ", 
                                  lubridate::hour(started_at), ":", minute),
                           "Ymd HM")) %>% 
  group_by(started_at, start_lng_sector, start_lat_sector, member_casual) %>% 
  summarise(n = n()) %>% 
  # summarise(avg_trip_mins = mean(trip_minutes), n_trips_sector = n())
  mutate(total_trips = sum(n),
         p_casual = n / total_trips) %>%  
  filter(member_casual == "casual") %>%
  select(-c(member_casual, n))
...
p <- volume %>% 
  ggplot(aes(start_lng_sector, start_lat_sector, fill = total_trips, group = started_at)) +
  geom_sf(data = boundaries_chi, mapping = aes(), inherit.aes = FALSE, fill="grey60") +
...
FPS = 30;W = 600;H = 660;DTL = 3;S = paste0(W,"x",H);NFRAMES = 3*24*1*5 # minutebins*hours*days*framesperbin
animate(plot = p, fps = FPS, nframes = NFRAMES, width = W, height = H, detail = DTL)
# anim_save("img/traffic-volume-map-bin2d.mp4", p, fps = FPS, nframes = NFRAMES,
#           width=W, height=H, detail=DTL,
#           renderer = ffmpeg_renderer(options = list(s = S, b = "1200k")))
# anim_save("img/traffic-volume-map-bin2d.gif", p, fps = FPS, detail = DTL, nframes = NFRAMES, width=W, height=H)
```
...
FPS = 24; W = 650; H = 680; DTL = 2; NFRAMES = 6*24*3 # bins per hour * 24 hours * (transition + state)
animate(plot = p, fps = FPS, nframes = NFRAMES, width=W, height=H, detail = DTL)
